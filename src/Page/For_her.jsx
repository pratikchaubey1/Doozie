...
import HerImg from '../assets/Her.png'
import { ProductContext } from '../Context/ProductContext/ProductContext'

function For_her() {
  const {her , sether} = useContext(ProductContext)
  return (
      <div className='bg-amber-50'>    
            <div className='bg-amber-50'>
              {/* Heading */}
              <h1 className="bg-gray-100 px-4 py-2 mt-20 text-xl font-mono flex items-center gap-2">
                For Her <span className="text-sm text-gray-600">({her.length} items)</span>
              </h1>
              ...
              <div className="mt-10 px-4 sm:px-10">
                <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-6">
                  ...
                  {/* Next 8 cards */}
                  {her.slice(10, 18).map((item, index) => (
                    <div
                      key={index}
                      onClick={() => sether(item)}
                      className="overflow-hidden hover:scale-105 transform transition duration-300 ease-in-out mx-auto w-[90%] sm:w-full"
